...
const Worlddailyconfirmedcases =()=>{
    ...
    useEffect(()=>{
        ...
        .then(data=>{
            const dailyConfirmedCases= data.map(i => {
                var temp={}
                temp["Date"]=parsedate(i.Data_date.slice(0,10));
                temp["DailyConfirmed"]=i.DailyConfirmed;
                return temp;
            })
            ...
            .domain(d3.extent(dailyConfirmedCases, function(d){return d.Date}))
			.range([0,width])
            ...
            const max= d3.max(dailyConfirmedCases,function(d){return d.DailyConfirmed})
...
export default Worlddailyconfirmedcases;