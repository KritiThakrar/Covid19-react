...
const Worldtotalconfirmedcases =()=>{
    ...
    useEffect(()=>{
        ...
        .then(data=>{
            const confirmedCases= data.map(i => {
                var temp={}
                temp["Date"]=parsedate(i.Data_date.slice(0,10));
                temp["Confirmed"]=i.TotalConfirmed;
                return temp;
            })
			...
			const svg = d3.select(d3worldtotal.current)
                        ...
                        .style('background-color','#d3f4ff')
						.append('g')
							.attr('transform', 'translate('+ margin.left + ',' + margin.top + ')');

            const x = d3.scaleTime()
            .domain(d3.extent(confirmedCases, function(d){return d.Date}))
			.range([0,width])
            ...
            const max= d3.max(confirmedCases,function(d){return d.Confirmed})
...
export default Worldtotalconfirmedcases;