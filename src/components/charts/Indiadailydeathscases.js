...
const Indiadailydeathscases =()=>{
    ...
    useEffect(()=>{
        ...
        .then(data=>{
            const dailyDeathsCases= data.map(i => {
                var temp={}
                temp["Date"]=parsedate(i.Data_date.slice(0,10));
                temp["DailyDeaths"]=i.DailyDeaths;
                return temp;
            })
            // console.log(dailyDeathsCases);
            ...
            .domain(d3.extent(dailyDeathsCases, function(d){return d.Date}))
			.range([0,width])
            ...
            const max= d3.max(dailyDeathsCases,function(d){return d.DailyDeaths})
            ...
            svg.append('path')
            .datum(dailyDeathsCases)
            .attr('fill','none')
            .attr('stroke','black')
            .attr('stroke-width',3)
            .attr('d', d3.line()
            .x(function(d){return x(d.Date)})
            .y(function(d){return y(d.DailyDeaths)})
            )
            svg.append('text')
			.attr('x',(width/2))
			.attr('y', (margin.top/5))
...
export default Indiadailydeathscases;